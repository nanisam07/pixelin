...
// Assuming validateCropImage is imported from your services, add it here if needed:
// import { validateCropImage } from "../../services/aiService";
...
export default function ScannerCard({ onScanStart, onScanComplete }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      // Moved image loading & validation inside the conditional block where imageUrl exists
      const img = new Image();
      img.src = imageUrl;
      img.onload = async () => {
        try {
          // Assuming validateCropImage is globally available or imported
          const predictions = await validateCropImage(img);
          ...
          console.log("Is it a plant?", isPlant);
          // You can now use `isPlant` here to conditionally cancel the scan if it's not a plant!
