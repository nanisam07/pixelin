...
import { translateText, getStaticTranslation } from "../../lib/translate";
import {
  identifyPlant,
...
export default function ScannerCard({ onScanStart, onScanComplete }) {
  ...
  const processLocalizationPipeline = async (resultData, targetLanguage) => {
    setIsTranslating(true);
    try {
      const translatedCrop = await translateText(resultData.crop, targetLanguage);
      const translatedIssue = await translateText(resultData.issue, targetLanguage);
      
      const productsList = resultData.recommendedProductIds || [];
      const translatedProducts = await Promise.all(
        productsList.map(async (id) => {
          const item = localProductCatalog[id];
          if (!item) return null;
          
          const pName = await translateText(item.name, targetLanguage);
          const pDesc = await translateText(item.description, targetLanguage);
          const pInst = item.instruction ? await translateText(item.instruction, targetLanguage) : "";
      ...
      const staticHeaderResult = await translateText("Diagnostic Analysis Completed", targetLanguage);
      const staticHeaderConfidence = await translateText("Confidence Rating", targetLanguage);
      const staticHeaderProducts = await translateText("Treatment Directives & Prescribed Stock", targetLanguage);
      ...
      setTranslatedContent({
        ...
        uiHeaders: {
          resultHeader: staticHeaderResult,
          confidenceHeader: staticHeaderConfidence,
          productsHeader: staticHeaderProducts
        }
      });
    } catch (err) {
      console.error("Localization engine compilation error, running fallback:", err);
    } finally {
