...
import { understandFarmerQuery } from "../services/geminiService";
...
export default function Home() {
  // --- Lang Config ---
  const languageConfig: Record<string, { code: string; label: string; placeholder: string; voiceCode: string; defaultGreeting: string }> = {
    te: {
      code: "te-IN",
      label: "Telugu",
      placeholder: "మీ పంట, వ్యాధుల గురించి అడగండి...",
      voiceCode: "te-IN",
      defaultGreeting: "నేను మీకు ఎలా సహాయం చేయగలను?"
    },
    hi: {
      code: "hi-IN",
      label: "हिन्दी",
      placeholder: "फसलों, बीमारियों के बारे में पूछें...",
      voiceCode: "hi-IN",
      defaultGreeting: "मैं आपकी क्या सहायता कर सकता हूँ?"
    },
    en: {
      code: "en-US",
      label: "English",
      ...
      voiceCode: "en-US",
      defaultGreeting: "How can I help you today?"
    },
    kn: {
      code: "kn-IN",
      label: "ಕನ್ನಡ",
      placeholder: "ಬೆಳೆಗಳು, ರೋಗಗಳ ಬಗ್ಗೆ ಕೇಳಿ...",
      voiceCode: "kn-IN",
      defaultGreeting: "ನಾನು ನಿಮಗೆ ಹೇಗೆ ಸಹಾಯ ಮಾಡಲಿ?"
    },
    ml: {
      code: "ml-IN",
      label: "മലയാളം",
      placeholder: "വിളകളെക്കുറിച്ചും രോഗങ്ങളെക്കുറിച്ചും ചോദിക്കൂ...",
      voiceCode: "ml-IN",
      defaultGreeting: "ഞാൻ నిങ്ങളെ എങ്ങനെ സഹായിക്കണം?"
    }
  };
  ...
  const [diagnosis, setDiagnosis] = useState<{ crop: string; problem: string } | null>(null);
  ...
  useEffect(() => {
    const saved = localStorage.getItem("pspl_selected_language");
  ...
  const handleLanguageChange = (langKey: string) => {
    ...
    localStorage.setItem("pspl_selected_language", langKey);
  };
  ...
  const speakText = (text: string) => {
    if (typeof window !== "undefined" && "speechSynthesis" in window) {
      ...
      const currentConfig = languageConfig[selectedLanguage] || languageConfig.en;
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.lang = currentConfig.voiceCode;
  ...
  const startVoice = () => {
    ...
    SpeechRecognition.startListening({
      continuous: false,
      language: currentConfig.code,
    });
  };

  // --- Route Redirection / Navigation Logic ---
  const handleSearch = async (customQuery?: string) => {
    const queryToEvaluate = (customQuery || searchQuery || "").toLowerCase().trim();

    if (!queryToEvaluate) return;

    if (
      queryToEvaluate.includes("scan") ||
      queryToEvaluate.includes("disease") ||
      queryToEvaluate.includes("leaf") ||
      queryToEvaluate.includes("crop")
    ) {
    ...
    try {
      ...
      const products = getRecommendations(ai.crop, ai.problem, cropProducts);

      setDiagnosis(ai);
      setRecommendations(products);

      setTimeout(() => {
        document.getElementById("recommendations-display")?.scrollIntoView({
          behavior: "smooth",
        });
      }, 200);

    } catch (error) {
      console.error("Search integration pipeline encountered an error:", error);
    }
  };
  ...
  const activePlaceholder = languageConfig[selectedLanguage]?.placeholder || languageConfig.en.placeholder;
          ...
          <div>
            ...
            <div className="flex flex-col gap-5 text-lg font-medium">
              ...
              <div>
                ...
                >
                  <button onClick={() => selectCropAndScroll("paddy")} className="text-left hover:text-accent">
                    Paddy Solutions
                  </button>
                  <button onClick={() => selectCropAndScroll("cotton")} className="text-left hover:text-accent">
                    Cotton Solutions
                  </button>
                  <button onClick={() => selectCropAndScroll("vegetables")} className="text-left hover:text-accent">
                    Vegetable Solutions
                  </button>
                </div>
              </div>
        ...
        <div className="relative z-10 h-full w-full max-w-6xl mx-auto px-6 md:px-12 flex flex-col justify-center items-center text-center text-white select-none pt-16">
          ...
          <div className="animate-fade-up delay-3 w-full max-w-2xl mx-auto flex flex-col items-center">
                ...
                {Object.entries(languageConfig).map(([key, lang]) => (
                  <button
                    ...
                    className={`transition-colors duration-200 font-medium ${
                      selectedLanguage === key ? "text-accent font-bold scale-105" : "text-white/70 hover:text-white"
                    }`}
                  >
                    {lang.label}
                  </button>
            ...
            <div className="w-full bg-white rounded-full shadow-2xl border border-gray-200/80 p-1.5 flex items-center transition-all duration-300 focus-within:ring-4 focus-within:ring-accent/30">
              ...
              <div className="flex items-center gap-1 pr-1">
                ...
                <button
                  ...
                  className="h-9 px-4 md:h-10 md:px-5 rounded-full bg-primary text-white font-label font-bold text-xs md:text-sm hover:bg-primary-dark transition-all duration-200 shadow-sm shadow-primary/20"
                >
                  Search
                </button>
              </div>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2 text-xs text-white/90">
              ...
              {[
                { label: "🌾 Paddy Disease", val: "Paddy Disease" },
                { label: "🍅 Tomato Wilt", val: "Tomato Wilt" },
                { label: "🐛 Pest Control", val: "Pest Control" },
                { label: "🧪 Fertilizers", val: "Fertilizers" }
              ].map((tag, idx) => (
                <button
                  key={idx}
                  onClick={() => {
                  ...
                  className="bg-white/10 hover:bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full border border-white/5 transition-all duration-200"
                >
                  {tag.label}
                </button>
              ))}
      ...
      {diagnosis && (
        <section id="recommendations-display" className="py-16 px-6 md:px-12 bg-white border-b border-gray-100">
          <div className="max-w-6xl mx-auto">
            <div className="bg-lightBg rounded-3xl p-8 md:p-12 shadow-sm border border-primary/5">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pb-6 border-b border-gray-200">
                <div>
                  <span className="px-3 py-1 bg-primary text-white text-xs font-label uppercase font-bold rounded-full tracking-wider">
                    Pixelin AI Diagnostic Output
                  </span>
                  <h3 className="font-headline font-bold text-2xl md:text-3xl text-primary-dark mt-2">
                    Identified Crop: <span className="capitalize text-accent-dark">{diagnosis.crop || "Unknown"}</span> 
                    {diagnosis.problem && ` · Issue: ${diagnosis.problem}`}
                  </h3>
                </div>
                <button 
                  onClick={() => { setDiagnosis(null); setRecommendations([]); }} 
                  className="text-gray-400 hover:text-gray-600 text-sm font-label flex items-center gap-1"
                >
                  Clear Results <i className="fa-solid fa-xmark"></i>
                </button>
              </div>
              ...
              {recommendations.length > 0 ? (
                  ...
                  {recommendations.map((prod, idx) => (
                    <div key={idx} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
                      <div>
                        ...
                        <h4 className="font-headline font-bold text-xl text-primary-dark mb-1">{prod.name}</h4>
                        <p className="font-body text-xs text-gray-500 mb-3 italic">"{prod.tagline}"</p>
                        <p className="font-body text-sm text-gray-600 leading-relaxed mb-4">{prod.desc}</p>
                      </div>
                      
                      <div className="border-t border-gray-100 pt-4 mt-4">
                        {prod.targets && (
                          <div className="mb-2">
                            <span className="text-xs font-bold text-primary block uppercase font-label mb-1">Target Pests:</span>
                            <div className="flex flex-wrap gap-1">
                              {prod.targets.map((tgt, i) => (
                                <span key={i} className="text-[10px] text-gray-500 bg-gray-100 px-2 py-0.5 rounded">{tgt}</span>
                              ))}
                            </div>
                          </div>
                        )}
                ...
                <div className="text-center py-8 bg-white rounded-2xl border border-dashed border-gray-200">
                  <p className="font-body text-gray-500 text-sm">
                    No matching specialized target formulations were found inside our local catalog inventory for this precise issue. Please consult our Farm Advisory service.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-gray-700">
                    {cropProducts[activeTab as keyof typeof cropProducts].target}
                  </span>
                </div>
              </div>
              <span className="px-3 py-1 bg-primary-muted text-primary text-xs font-bold font-label uppercase rounded-full">
                {cropProducts[activeTab as keyof typeof cropProducts].items.length} Key Products
              </span>
            </div>

            {/* Product Cards Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {cropProducts[activeTab as keyof typeof cropProducts].items.map((prod, idx) => (
                <div
                  key={idx}
